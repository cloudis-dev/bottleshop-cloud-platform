...
import 'package:delivery/src/core/utils/streamed_items_state_management/data/items_state.dart';
...
class CreateStreamMustCreateNewInstanceException implements Exception {
  ...
  CreateStreamMustCreateNewInstanceException({this.message = 'createStream method have to create a new stream!'})
      : super();
...
class ItemsStreamHandler<T, E> {
  ...
  ItemsStreamHandler.listen({
    ...
    /// so there is no need to update the current state (also there are cases where this is incorrectl10n.
    ...
    void onError(
      ...
      if (isInitialBatch) {
        ...
        onItemsStateUpdated(
          getCurrentItemsState(l10n.copyWith(status: ItemsStateStatus.error),
          isInitialStreamBatch: isInitialBatch,
          hasError: true,
        );
        ...
        l10n.catchError((err, stacktrace) {
          onErrorCallback(err, stacktrace);
        });
      }
    }
    ...
    StreamSubscription<ItemsStateStreamBatch<T>> createSubscription(
      ...
      return stream.listen(
        ...
        onError: (err, stacktrace) => onError(err, stacktrace, createSubscription),
      );
    }
  ...
  ItemsState<T> _createUpdatedState(
    ...
    if (data.isEmpty) {
      currentItemsState = currentItemsState.copyWith(status: ItemsStateStatus.allLoaded);
    } else {
      final removedItems = _getItemsByChangeType(data, ChangeStatus.removed);
      currentItemsState =
          currentItemsState.copyWith(items: itemsHandler.removeItems(currentItemsState.items, removedItems));

      final modifiedItems = _getItemsByChangeType(data, ChangeStatus.modified);
      currentItemsState =
          currentItemsState.copyWith(items: itemsHandler.updateItems(currentItemsState.items, modifiedItems));

      final List<T> addedItems = _getItemsByChangeType(data, ChangeStatus.added);
      currentItemsState = currentItemsState.copyWith(
        items: itemsHandler.addItems(currentItemsState.items, addedItems),
      );

      currentItemsState = currentItemsState.copyWith(status: ItemsStateStatus.waiting);
    }
  ...
  List<T> _getItemsByChangeType(
    ...
    ChangeStatus changeType,
      ...
      data.where((element) => element.value1 == changeTypel10n.map((e) => e.value2l10n.toList();
}
